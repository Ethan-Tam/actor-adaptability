...
class PieChart {
  ...
  update() {
    ...
    if (vis.selectedActor == null) {
      ...
      vis.data = d3
        .pie()
        .value(d => d.count)
        .sort((a, b) => vis.genreMap[a.genre] - vis.genreMap[b.genre])(genreData);
      vis.title = vis.selectedActor.actor;
    }

    vis.polyline = vis.chart.select('.lines').selectAll('polyline').data(vis.data);
  ...
  arcTween(arc) {
    return d => {
      let vis = this;
      let interpolateStart = d3.interpolate(vis.lastAngles[d.data.genre].startAngle, d.startAngle);
      let interpolateEnd = d3.interpolate(vis.lastAngles[d.data.genre].endAngle, d.endAngle);
      return t => {
