...
5. #### Reflection
   ...
   - How the project has developed
      ...
      - Changes Between Proposal to M2
         - We made some initial progress, however after talking to TAs and Professor Munzner, we found our initial chord diagram visualization to be ineffective and confusing in acheiving our task. We originally represented each actor with a set of chords (as each individual chord signified a switch in movie genres of a given actor) which did not scale well and did not allow for good UX. Even after bundling and setting each movie with a single main genre, it was still extremely difficult to read (Figure 1). We ended up creating something that looked visually appealing by hiding the chords behind a band but it did not resolve our confusing chord mark (Figure 2). Therefore, we decided to pivot to representing each actor with a node in a network diagram (Figure 3). 
      - Figure 1
